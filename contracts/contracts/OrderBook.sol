...
contract OrderBook {
    ...
    function listTicket(uint256 tokenId, uint256 price) external {
        require(lotteryTicket.ownerOf(tokenId) == msg.sender, "Not token owner");
        require(tokenToOrder[tokenId] == 0, "Token already listed");
        require(price > 0, "Price must be greater than 0");
        
        // 委托授权：允许合约转移该NFT
        lotteryTicket.approve(address(this), tokenId);
        ...
        // 添加到项目选项的订单列表
        uint256 projectOptionKey = ticketInfo.projectId * 1000 + ticketInfo.optionIndex;
        ...
        emit OrderCreated(orderId, msg.sender, tokenId, ticketInfo.projectId, ticketInfo.optionIndex, price);
    }
     ...
     */
    function _executeOrder(uint256 orderId, address buyer, uint256 paymentAmount) internal {
        Order storage order = orders[orderId];
        require(order.isActive, "Order not active");
        ...
        if (useERC20Payment) {
            ...
            require(betToken.transferFrom(buyer, order.seller, order.price), "ERC20 transfer failed");
        } else {
        ...
        emit OrderFilled(orderId, order.seller, buyer, order.tokenId, order.price);
    }
     ...
     */
    function buyAtBestPrice(uint256 projectId, uint256 optionIndex) external payable {
        ...
        require(msg.value == bestPrice || (useERC20Payment && msg.value == 0), "Incorrect payment amount");
     ...
     */
    function getOrderBook(uint256 projectId, uint256 optionIndex) 
        external 
        view 
